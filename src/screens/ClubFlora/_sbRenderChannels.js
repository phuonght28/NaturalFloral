...
export default class ChannelList extends Component {
  ...
  _onOpenChannelPress = (propsItem, channelType) => {
    if (channelType == 'group') {
      this.props.navigation.navigate("GroupChannel", { user: propsItem, updateData: this.updateData })
    }
    else {
      this.props.navigation.navigate('Chat', propsItem)
    }
  }
  render() {
    let renderListChannels = <ActivityIndicator />
    const { channels, channelType } = this.props
    if (channels && channelType == 'open') {
      renderListChannels = channels.map((channel, index) => {
        return (
          <TouchableOpacity key={index} style={styles.button} onPress={() => this._onOpenChannelPress(channel, channelType)}>
            <View style={{ flexDirection: 'row', marginBottom: 10, flex: 1 }}>
              <Image style={styles.cover} source={{ uri: channel.coverUrl }} />
    ...
    else if (channels && channelType == 'group') {
      renderListChannels = channels.map((channel, index) => {
        // const isSender = channel.lastMessage._sender.userId == this.currentUser.userId ? true : false
        //const isSeen = channel.unreadMessageCount > 0 ? 'check' : 'check-all'
        const isNew = channel.unreadMessageCount > 0 ? '600' : '300'

        return (
          <TouchableOpacity key={index} style={styles.button} onPress={() => this._onOpenChannelPress(channel.propsUser, channelType)}>
            <View style={{ flexDirection: 'row', marginBottom: 10, flex: 1 }}>
              <View style={{ flex: 0.1 }}>
                <Image style={styles.cover} source={{ uri: channel.coverUrl }} />
              </View>
              <View style={{ flex: 0.7 }}>
                ...
                <Text numberOfLines={1} style={[styles.text, { fontWeight: isNew }]}>{channel.lastMessage.message}</Text>
              </View>
              <View style={{ flex: 0.2, alignItems: 'flex-end' }}>
                <Text numberOfLines={1} style={[styles.text, styles.time, { fontWeight: isNew }]}>{timestampToTime(channel.lastMessage.createdAt)}</Text>

                {channel.unreadMessageCount > 0 &&
                  <View style={{ backgroundColor: '#FF3030', width: 16, height: 16, borderRadius: 16, alignItems: 'center' }}>
                    <Text adjustsFontSizeToFit style={{ color: '#FFFFFF', backgroundColor: 'transparent', textAlign: 'center', lineHeight: 14, fontSize: 10, fontWeight: '600' }}>{channel.unreadMessageCount}</Text>
                  </View>
                }
                {/* {isSender && <Icon style={{ color: '#92C7A9', fontSize: 16 }} name={isSeen} type="MaterialCommunityIcons" />} */}
              </View>
            </View>
          </TouchableOpacity>
        )
