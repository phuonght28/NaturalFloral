...
export default class Discussion extends Component {
  ...
  async componentWillMount() {
    ...
    this.setState({ FAVORITE }, () => {
      ...
      this._getGroupChannelList(userSendbird.userId)
    })
  }
  sendbirdOpenChannelListQuery() {
    const self = this
    const openChannelListQuery = this.sendbird.OpenChannel.createOpenChannelListQuery()
    openChannelListQuery.next(function (channels, error) {
      if (error) {
        console.log('Profile/SendBird OpenChannel error', error)
      }
    ...
    this.sendbirdAction.getOpenChannelList(isInit)
      .then(openChannelList => {
        openChannelList.forEach(channel => {
          console.log('_getOpenChannelList channel ================================', channel)
        })
        openChannelList.sort(function (a, b) { return (a.name > b.name) ? 1 : ((b.name > a.name) ? -1 : 0); })
        self.setState({ openChannelList }, () => {
          self.setState({ isFetching: false })
        })
      })
      .catch(error => {
        console.log('_getOpenChannelList error', error)
      });
  }
  _getGroupChannelList(userId, isInit = false) {
    const self = this
    this.sendbirdAction.getGroupChannelList(userId, isInit)
      .then(recentGroupChannel => {
        if (recentGroupChannel) {
  ...
  render() {
    ...
    if (!this.state.isFetching && this.state.openChannelList) {
      const { openChannelList, FAVORITE } = this.state
      const clubfloraChannel = openChannelList.find((channel) => channel.url === 'clubflora')
      channelsFAVORITE.push(clubfloraChannel)
      FAVORITE.map((item) => {
        item = item.replace('-', '')
        channelsFAVORITEURL.push(item)
      })
      openChannelList.map((channel) => {
